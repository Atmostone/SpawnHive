...
| GET | `/api/experiments/{id}/bundle?selection=&method=` | **SPA-90 — the frozen reproduction bundle**: a `.tar.gz` an outside reader recomputes the numbers from, with a clean checkout and **no database, object store or provider calls**. Holds `manifest.json`, `experiment.json`, `runs.jsonl`, `records.jsonl` (every profile blob), `annotations.jsonl` (typed, SPA-85 — each flagged `selected`, the calibration choice the database made, carried rather than re-derived offline), `expected.json` and `blobs/records/` (the MinIO record archives — the half a Postgres dump loses). Verify with `python -m app.cli.bundle verify --bundle <file>`. The verdict is **`ok`** — `reproduced` (the numbers) **and** `complete` (the evidence behind them). They are separate because every figure recomputes from Postgres alone, so an archive that lost its record blobs would otherwise report `reproduced: true` — precisely the failure this bundle exists to insure against. `manifest.blobs` is a sha256 index over every archived blob, and `counts.expected_record_blobs` says how many a whole bundle must hold; export **fails closed** (409) rather than shipping without one. «Reproduced» itself has two levels: the named `headline` metrics (calibration, `axis_reliability`, `outcome_axis_reliability`, `rq2`, `judge_discrimination`, `summary`, `trusted`) — a mismatch is a **failure**; and `full_report_sha256` over the whole canonical report — a mismatch is a **warning with a key-path diff** against the canonical report the bundle carries in `expected.report` (stored, not merely hashed — a digest can say *that* something moved, only the report can say where), so a `SCHEMA_VERSION` bump does not retroactively make every old bundle unreproducible while drift elsewhere still cannot pass silently. The report is deterministic apart from `generated_at`, the only field dropped before hashing. The bundle is **verified as it is built** — the report is computed both through `compute_report` and through the offline path, and a disagreement returns **409** with the differing paths instead of writing an archive, because a bundle that does not already reproduce is a claim rather than an artifact. `manifest.replay` states the honest name — **best-effort input replay**, not exact state replay: template versions, provider endpoints and image digests are not pinned — and `manifest.replay.derivation` names which sections are re-derived from the profiles (`trusted.*`, the reliability badges, calibration) versus replayed from the denormalized `experiment_runs.weighted_score` (`summary`, `rq2`, `judge_discrimination`, `leaderboard`, `significance`). `manifest.platform` names the checkout that produced the bundle, from `SPAWNHIVE_GIT_SHA` baked in at image build (`GIT_SHA=$(git rev-parse HEAD) docker compose build api`) — and says `source: unknown` with a note when it was not, since «unpinned» and «nobody filled this in» call for different amounts of trust. `manifest.coverage` aggregates the per-run E-20 snapshots so a reader sees which conditions were pinned; `manifest.environment_at_export` carries `config_drift` and `calibration_fingerprint` as recorded-not-recomputed, since both describe the live stand. Agent log archives are **CLI-only** (`--with-traces`): they are the bulk of the object store and no number depends on them; the manifest states the tier either way, so «no traces» is never read as «there were none» |
