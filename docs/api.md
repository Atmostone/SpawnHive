...
| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/v1/agent-log/{task_id}` | Agent → orchestrator chunk ingest. Bearer agent_token + idempotency. Body: `{chunk_seq, content (≤256 KB), tool_name?, arguments?, arguments_truncated?, tool_call_id?, part_index=0, part_total=1, idempotency_key}`. Returns `{status: ok\|duplicate, chunk_seq}`. Auto-remap of chunk_seq on retry collision. `arguments` is the call's parameters (SPA-86) — re-clipped server-side, shortening long string values while keeping every key; `tool_call_id` + `part_index` identify the parts of one split output so the cleaned trace joins them into a single step. | Archived to MinIO with `created_at` so the cleaned trace can be re-ordered after compaction (SPA-113).
