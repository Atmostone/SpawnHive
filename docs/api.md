...
| POST | `/api/experiments/{id}/retry-failed` | **owner/admin** — re-queue every `experiment_run` row currently in `failed`/`preprocessing`/`evaluating` (Toolathlon external-eval states are excluded — see `d8e9f0a1b2c3`) back to `pending`, so the next `experiment_run_tick` re-claims them. Idempotent. 409 if the experiment is not in a terminal or terminal-pending state |
| POST | `/api/experiments/{id}/add-config` | **owner/admin** — append a new configuration to a *running* or *paused* experiment. Body `{config_key?, label?, orchestrator?, template_id?, model_id?, temperature?, seed?, soul_md?, tools_override?, memory_mode?}` (validated + fingerprinted the same way as `POST /api/experiments`); new cells are pre-created as `pending` and drained by the existing tick. 409 on duplicate fingerprint or invalid transition |
| DELETE | `/api/experiments/{id}/configs/{config_key}` | **owner/admin** — remove a configuration from a *paused* experiment (drops its `experiment_runs` rows + their `task_id` references). 409 while running; cancel/pause first |
...
| GET | `/api/quality/judge-calibration/badge` | Compact trust badge: `{calibrated, n_humans, sample_size, overall_kappa, judge_config_key, version, passed, created_at}` (or `{calibrated:false}` until the first run) — renders "judge calibrated against N humans, κ=X.X". SPA-79 adds a rank-aware `directional` rescue: an axis with κ<0.4 but Spearman ρ≥0.5 keeps its ordering trustworthy (usable for A/B comparisons, not absolute scores) instead of being marked `unreliable` |
