...
| failure_type | VARCHAR(32) | NULL | WHY it failed, when it did (SPA-87). `status` says only *that* it did, and a dozen writers set it. Vocabulary in `app/utils/failures.py`: `llm_rate_limit`, `llm_auth`, `llm_transient`, `infra` (all four = infrastructure decided the outcome, so the experiment report **excludes** the run from its aggregates), `llm_error`, `cap_hit`, `timeout`, `agent` (all four counted normally). NULL = not classified — an older agent image, a path that never reported — and counts as ordinary data. Set from the structured `failure` facts in the agent's `failed` webhook, cleared when a task goes back to `ready` for a retry. A contaminating value is **merged, never assigned** — it cannot be demoted by a later, ordinary reason (a quota that made the run stop measuring the model outranks the cap-hit that happened afterwards); `NULL` never overwrites; a retry clears only the reason of the attempt that just ended, keeping a contamination recorded **before** the agent ran — that one describes the condition, and the condition survives the retry because the substituted template is pinned on the task. Also written by the harness where no agent reports: a wall-clock timeout, a spawn or workspace-configuration failure, a Toolathlon preprocess collapse, and a decomposition parent inheriting a child's infrastructure failure (under `orchestrator:on` the run denormalizes from the parent, so without that the reason is lost). Distinct from the LLM-judged classes in `quality_records.failure_profile`: this is what killed the process, observed; those are how the agent misbehaved, judged |
| condition_contaminated | BOOL | false | Whether that reason describes the **condition** the next attempt will inherit, or only the attempt that failed (SPA-115). Set solely by the orchestrator's degrade path, which is the only code that knows it could not reach its LLM, substituted a template and **pinned** it — from downstream, that and an agent-side 429 are indistinguishable. A manual re-queue clears `failure_type` unless this is set; inferring it from `template_id` (as the guard first did) marks every orchestrated task, so an ordinary run that hit a quota kept its contamination forever and its next successful run was dropped from every aggregate |
...
| eval_config | JSONB | {} | Write-once (no update endpoint) and hashed into `input_fingerprint`, so it is what the experiment is committed to. Keys: `eval_mode` (`checker` \| `judge`), `trajectory` (default true), `failure_modes`, `judge_incomplete_runs`, `outcome_files_only`, `audit_outcome_judge_on_verifiable`, `judge_threshold`, `equivalence_margin`, `trace`. `judge_threshold` (SPA-87, 0–10) is the outcome-judge cut-off the report's verdict×judge quadrant is drawn at — a **pre-registration**, not a setting: fixed before any result exists, and changing it means cloning into a new experiment. `equivalence_margin` (SPA-62, >0 and ≤10) is the smallest difference in judge points worth calling a difference — the margin the report's equivalence (TOST) verdict is tested against, and pre-registered for the same reason: chosen after the data it would be a description of the result, not a test of it. Both are stamped with the project default at create time when the author names none, so a report's `margin_source: default` means «created before the field existed», not «nobody chose». `trace` (SPA-86) overrides the trim policy for this experiment's process judging: `{tool_output_token_cap, tool_args_token_cap, keep_tail_on_error, max_input_tokens}`, any cap `0` = no truncation; absent → the workspace settings apply. **The whole object is validated at create time** — an unknown key, a mistyped flag or an out-of-range threshold is a 400, because a key that does nothing is a lie about how the experiment was judged |
